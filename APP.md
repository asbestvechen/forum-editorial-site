...
- Equal-height directions gallery with a touch-friendly horizontal strip on mobile that no longer traps the page's vertical scroll, preserving the original seven service categories; the detail drawer exposes a scrollable chip list with local brand marks and text fallbacks for the full known partner/factory list, while the outer cards stay focused on the image, title, and category description.
