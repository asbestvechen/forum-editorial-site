...
- Visualizer route at `#/visualizer` with a first MVP for bathroom tile selection: a real-time Three.js room scene with orbit camera, lighting, bathtub/vanity/shower geometry, independently textured wall/floor surfaces, official manufacturer material cards, search/filtering, source links, and texture/catalog download links.
...
- `src/components/VisualizerPage.tsx` and `src/lib/materials.ts`: Three.js tile visualizer MVP, manufacturer-backed material catalog, room-surface application state, and source/download links for designers.
