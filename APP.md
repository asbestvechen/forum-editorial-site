...
- Visualizer route at `#/visualizer` with a GLTF-based photorealistic bathroom scene: PBR materials, orbit camera, environment and directional lighting with shadows, detailed bathtub/vanity/shower geometry, independently replaceable wall/floor surfaces, official manufacturer material cards, search/filtering, source links, and texture/catalog download links.
...
- `src/components/Bathroom3DScene.tsx`: GLTFLoader-based bathroom renderer with PBR scene lighting, orbit controls, shadows, and wall/floor texture replacement.
- `src/components/VisualizerPage.tsx` and `src/lib/materials.ts`: GLTF tile visualizer UI, manufacturer-backed material catalog, room-surface application state, and source/download links for designers.
- `public/assets/bathroom/bathroom_extended.gltf` + `bathroom.bin`: detailed GLTF 2.0 scene and external binary buffer; `bathroom.bin` is approximately 56 MB, so keep it intact and account for the download/memory cost before production publication.
