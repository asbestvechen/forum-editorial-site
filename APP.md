...
- Visualizer route at `#/visualizer` with a photorealistic product render of one upright tile slab on a low plinth: PBR-style studio lighting, controlled orbit camera, realistic surface textures, immediate material selection, wheel zoom isolated from page scrolling, and a catalog of 20 confirmed Emilceramica, 41zero42, and Florim variants with manufacturer/color/search filters and official source links.
...
- `src/components/VisualizerPage.tsx` and `src/lib/materials.ts`: tile-on-plinth visualizer UI, 20 confirmed manufacturer variants grouped by color, manufacturer/color/search filters, immediate selection, and official source/download links.
